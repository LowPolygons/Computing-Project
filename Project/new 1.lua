filehandling = {
	...
	reformat_fileReference = function(v)
		...
		return filehandling:reformatter(v)
	end,
	...
	fileReference = function(k,v)
		local message = filehandling:storeData(v, k, true)
		return "\t\t-fileReference : "..k.." = "..message
	end,
}
...
function filehandling:storeData(dataTable, fileName, returnOrSave)
	returnOrSave = returnOrSave or false
...
function filehandling:segmenter(filedata)
	...
	while not parsed do
		...
		if not segmentPresent and startOfLine then
			...
			if datatype ~= "fileReference" then
				local value = currentData:sub(valueStart+1, endOfLine-1)
			else
				value = currentData:find(name.."End", 1, true)
			end
			returnedValue = self["reformat_"..datatype](value)
			fildata = filedata:sub(endOfLine+1, filedata:len())
